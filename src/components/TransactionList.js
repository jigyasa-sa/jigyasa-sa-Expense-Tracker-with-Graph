...
const TransactionList = () => {
  const { transactions, deleteTransaction, update } =
    useContext(TransactionContext);
  const [arrow, setArrow] = useState(faArrowAltCircleDown);
  const [toggleList, setToggleList] = useState({ display: "none" });

  const changeArrow = () => {
    if (toggleList.display === "none") setToggleList({ display: "flex" });
    else setToggleList({ display: "none" });
      ...
      <div className="transaction-list">
        {transactions.map((transaction) => (
          <ul className="list" style={toggleList} key={transaction.id}>
            <li
              className="list-item-history"
              style={{ color: transaction.amount < 0 ? "red" : "" }}
            >
            ...
            <li className="list-item-actions">
              <span
                onClick={() => deleteList(transaction.id)}
                className="delete"
              >
                <FontAwesomeIcon icon={faTrash} />
              </span>
              <span
                onClick={() => updateList(transaction.id)}
                className="update"
              >
